...
function [psv,psv_CI,BS_sample] = preference_scale_value(standings,num_sti,num_param,tnum)
    % parameters
    B = 10000; % Repetition number in Bootstrap
    %tnum = 6; % trial number in each stimulus pair in conventional experiment

    % result
    psv = zeros(1,num_sti,num_param);
    psv_CI = zeros(num_sti,3,num_param);
    BS_sample = zeros(B,num_sti,num_param);
    ...
    %% experiment result
    mtx = standings.mtx;
    out_of_num = standings.out_of_num;
    num_greater = standings.num_greater;
    ...
    for i = 1:num_param
        ...
        estimated_psv = TNT_FCN_PCanalysis_Thurston(mtx(:,:,i), 0.005);
        ...
        [estimated_psv2,num_greater_v,out_of_num_v] = TNT_FCN_PCanalysis_ML(out_of_num(:,:,i), num_greater(:,:,i), cmbs, InitValues, method);
        ...
        % record data
        psv(:,:,i) = estimated_psv2;
        psv_CI(:,:,i) = ranges95_ml;
        BS_sample(:,:,i) = sv_ml; % bootstrap sample
