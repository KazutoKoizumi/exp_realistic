...
% パス
pass.data = strcat('../data/exp_realistic/pre/',sn);
exp_record = sprintf('../data/exp_realistic/pre/%s/record_%s.txt', sn,sn);
...
% 予備実験で使用するパラメータ
param.light = 1; % 1:面光源, 2:環境照明
param.rough = 2; % roughness : 1:0.05, 2:0.1, 3:0.2

% 試行数
trial.num_pair_response = 2; % 1種の刺激対に対する被験者1人あたりの応答数
trial.num_pair = (object.hue_pair + object.hue_metal_pair) * numel(param.light) * numel(param.rough); % 刺激対の総数
trial.all = trial.num_pair * trial.num_pair_response;
trial.session = trial.all / (numel(param.light) * numel(param.rough) * trial.num_pair_response); % 1セッションの試行数
trial.trash = 10;
...
for i = 1:object.material_num
    for j = param.light(1):param.light(end)
        for k = param.rough(1):param.rough(end)
            if i == 1
